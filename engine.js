...
export function associationPeriod(evidence){
 ...
 return {first:dated.map(e=>e.startDate).sort()[0]||'',last:dated.map(e=>e.endDate).sort().at(-1)||'',overlaps:dated.some(e=>e.startDate<='202612'&&e.endDate>='201501')?'Yes':dated.length&&dated.length===evidence.length?'No':'Unknown'};
}
...
export function makeRow(atc,name='',inventorySource='Entered ATC code'){
 return {atc,name,inventorySource,ids:[],concepts:[],products:[],ndcs:[],notes:[],errors:[],stage:'Waiting',reason:'',done:false,enriched:false,productsDone:0,currentLookupComplete:false,coverage:'current',mappingSource:RX+mappingPath(atc)};
}
...
export async function enrichSubstance(row,api,onUpdate=()=>{}){
 ...
 const data=await api.rx(mappingPath(row.atc));
 ...
 for(const id of row.ids){
  try{const d=await api.rx('/rxcui/'+id+'/properties.json'),p=d.properties;
 ...
 for(const c of ingredients){
  try{const d=await api.rx(productPath(c.rxcui));
...
export async function retrieveNDCs(row,api,onUpdate=()=>{},coverage='current'){
 ...
 await parallel(row.products,3,async product=>{
  ...
  try{
   try{
    const data=await api.rx(ndcPath(product.rxcui));
   ...
   if(coverage==='history'){
    try{
     const data=await api.rx(historyPath(product.rxcui)),source=RX+historyPath(product.rxcui);
 ...
 if(coverage==='history'&&row.ndcs.length&&!row.ndcs.some(n=>n.current)&&!row.errors.length){
  ...
  row.reason='No current NDC associations; codes were returned by the history API.'+(period.last?' Latest recorded association: '+period.last.slice(0,4)+'-'+period.last.slice(4)+'.':'')+(period.overlaps==='No'?' No returned association overlaps 2015–2026.':'');
 }
...
export function substanceTable(rows,meta={}){return {headers:['ATC level 5','Chemical substance','Ingredient RxCUI(s)','RxNorm concept name(s)','Ingredient term type(s)','Related product count','Unique NDC count','Status','Reason / notes','ATC-to-RxCUI API URL','ATC inventory source','RxNorm release','Retrieved UTC','Run status','NDC coverage','Current NDC count','History API only NDC count','NDC count with recorded association overlapping 2015–2026'],rows:rows.map(r=>{const active=r.concepts.filter(c=>c.accepted),ndcs=consolidate([r]);return [r.atc,r.name,active.map(c=>c.rxcui).join('; '),active.map(c=>c.name).join('; '),active.map(c=>c.tty).join('; '),r.products.length,ndcs.length,r.stage,rowExplanation(r),r.mappingSource,r.inventorySource,meta.version,meta.retrieved,meta.status,coverageLabel(r.coverage),ndcs.filter(n=>n.current).length,r.currentLookupComplete?ndcs.filter(n=>!n.current).length:'Unknown',r.coverage==='history'?ndcs.filter(n=>associationPeriod(n.historyEvidence).overlaps==='Yes').length:'Not requested'];})};}
export function ndcTable(rows,meta={}){return {headers:['NDC11 - import as text','ATC level 5','Chemical substance','Ingredient RxCUI(s)','Product RxCUI(s)','Product name(s)','Product term type(s)','ATC-to-RxCUI API URL','Related-products API URL(s)','getNDCs API URL(s)','RxNorm release','Retrieved UTC','Run status','NDC coverage','Current association status for retrieved products','getAllHistoricalNDCs API URL(s)','Earliest first RxNorm association release','Latest last RxNorm association release','Any recorded association overlaps 2015–2026','RxNorm association evidence (JSON)'],rows:consolidate(rows).map(n=>{const period=associationPeriod(n.historyEvidence);return [n.ndc,n.atc,n.substance,n.ingredientRxcuis.join('; '),n.productRxcuis.join('; '),n.productNames.join(' | '),n.termTypes.join('; '),n.mappingSource,n.relatedSources.join(' | '),n.sources.join(' | '),meta.version,meta.retrieved,meta.status,coverageLabel(meta.coverage||rows.find(r=>r.atc===n.atc)?.coverage),ndcStatus(n),n.historySources.join(' | '),period.first,period.last,period.overlaps,JSON.stringify(n.historyEvidence)];})};}
