import {API,RX,coverageLabel,ndcStatus,associationPeriod,normalizeAtc,discover,enrichSubstance,retrieveNDCs,parallel,csv,consolidate,rowExplanation,substanceTable,ndcTable} from './engine.js';
...
function associationHTML(n){
 ...
 return '<span class="tag'+(!n.current?' warn':'')+'">'+esc(ndcStatus(n))+'</span>'+(n.historyEvidence.length?'<div class="subtle">First: '+esc(month(p.first))+'<br>Last: '+esc(month(p.last))+'<br>Overlaps 2015–2026: '+esc(p.overlaps)+'</div><details><summary>Association details</summary>'+n.historyEvidence.map(e=>'<p class="subtle">Product '+esc(e.productRxcui)+' · Associated RxCUI '+esc(e.associatedRxcui)+' · '+esc(e.route)+'<br>'+esc(month(e.startDate))+' → '+esc(month(e.endDate))+'</p>').join('')+'</details>':'<div class="subtle">'+(state.coverage==='current'?'History not requested':'No dated history returned')+'</div>');
}
...
function render(){
 ...
 const rows=state.rows.filter(r=>[r.atc,r.name,...r.concepts.flatMap(c=>[c.rxcui,c.name]),rowExplanation(r)].join(' ').toLowerCase().includes(term));
 $('substances-body').innerHTML=rows.map(r=>'<tr><td><strong class="code">'+esc(r.atc)+'</strong><div>'+esc(r.name||'Substance name not returned')+'</div></td><td>'+
  (r.concepts.map(c=>'<div class="concept-item"><strong class="code">'+link(c.source,c.rxcui)+'</strong> <span class="tag">'+esc(c.tty||'No active properties')+'</span><div>'+esc(c.name)+'</div></div>').join('')||'<span class="subtle">'+(r.enriched?'No active mapping':'Pending')+'</span>')+
  '</td><td><strong>'+r.products.length.toLocaleString()+'</strong> products<br><button class="row-button" data-ndc-atc="'+esc(r.atc)+'" '+(r.ndcs.length?'':'disabled')+'>'+new Set(r.ndcs.map(n=>n.ndc)).size.toLocaleString()+' NDCs</button>'+(state.coverage==='history'?'<div class="subtle">'+new Set(r.ndcs.filter(n=>n.current).map(n=>n.ndc)).size+' current · '+(r.currentLookupComplete?consolidate([r]).filter(n=>!n.current).length:'?')+' history API only</div>':'')+'</td><td class="status-cell"><span class="tag'+(r.errors.length?' warn':'')+'">'+esc(r.stage)+'</span><p class="subtle">'+esc(rowExplanation(r))+'</p>'+link(r.mappingSource,'ATC API response')+'</td></tr>').join('')||'<tr><td colspan="4" class="nodata">No matching substances.</td></tr>';
...
if(context?.registerTool){
 ...
 register({name:'read_atc_results',description:'Read the ATC summary, zero-result reasons and a sample of NDC mappings with current status and historical evidence.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:true},execute:async()=>({content:[{type:'text',text:JSON.stringify({...summary(),substances:state.rows.map(r=>({atc:r.atc,name:r.name,concepts:r.concepts,productCount:r.products.length,uniqueNdcCount:new Set(r.ndcs.map(n=>n.ndc)).size,status:r.stage,reason:rowExplanation(r)})),ndcSample:records().slice(0,20)})}]})});
